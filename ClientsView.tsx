        ...
        try {
          ...
          if (res) {
            ...
            if (res.clients && res.clients.length > 1) {
              ...
              const mappedClients = res.clients.slice(1).map((r: any[], index: number) => {
                let id = String(r[0] || `client-${index}`).trim();
                if (seenClientIds.has(id)) {
                  id = `${id}-${index}`;
                }
                seenClientIds.add(id);
                return {
                  id, name: r[1], phone: r[2], email: r[3], address: r[4], createdAt: r[5]
                };
              });
